...
function provisioner_end() {
  ...
  if [[ $PROVISION_SUCCESS -eq "0" ]]; then
    ...
    vvv_error " ! The <b>'${VVV_PROVISIONER_RUNNING}'</b><error> provisioner ran into problems, printing full log:"
    cat "${VVV_CURRENT_LOG_FILE}"
    vvv_error " ! The <b>'${VVV_PROVISIONER_RUNNING}'</b><error> provisioner ran into problems, the full log is printed above. It completed in <b>${elapsed}</b><error> seconds."
