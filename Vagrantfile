...
Vagrant.configure("2") do |config|
  ...
  config.trigger.after :up do |trigger|
    ...
    trigger.run_remote = { inline: "vagrant_up" }
  end
  config.trigger.before :reload do |trigger|
    trigger.name = "VVV Pre-Up"
    trigger.run_remote = { inline: "vagrant_halt" }
  end
  ...
  config.trigger.before :suspend do |trigger|
    ...
    trigger.run_remote = { inline: "vagrant_suspend" }
  end
  config.trigger.before :destroy do |trigger|
    trigger.name = "VVV Pre-Destroy"
    trigger.run_remote = { inline: "vagrant_destroy" }
  end
end
